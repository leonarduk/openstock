...
public class mcstocksapicomms {
	...
	public void av_testarcomms() {
		String conteudopagina = mwcomms.receberconteudopagina(
				"https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=MSFT&apikey=" + av_chave);
		communication.mcfuncoeshelper.mostrarmensagem(conteudopagina);
	}
	...
	public java.util.List<mccandle> av_receberstockcandlesintraday(String simbolo, String intervalo,
		 ...
		 * "Daily Prices (open, high, low, close) and Volumes", "2. Symbol": "MSFT",
		 * "3. Last Refreshed": "2018-12-21", "4. Output Size": "Compact",
		 * "5. Time Zone": "US/Eastern" }, "Time Series (Daily)": { "2018-12-21": {
		 * "1. open": "101.6300", "2. high": "103.0000", "3. low": "97.4600",
		 * "4. close": "98.2300", "5. volume": "111242070" }, "2018-12-20": { "1. open":
		 * "103.0500", "2. high": "104.3100", "3. low": "98.7800", "4. close":
		 * "101.5100", "5. volume": "70334184" },
		 */

		String urlquery = "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=" + simbolo
				+ "&interval=" + intervalo + "&outputsize=" + outputsize + "&apikey=" + av_chave;
		...
		for (int i = 0; i < candlesjsontimestamparray.length(); i++) {
			...
			// public mccandle(String tsstr, String ostr, String hstr, String cstr, String
			// lstr, String vstr)
			mccandle candleatual = new mccandle(timestampcandleparaclasse, opencandle, highcandle, closecandle,
					lowcandle, volumecandle);
	...
	public java.util.List<mccandle> av_receberstockcandlesdaily(String simbolo, String outputsize) {
		String urlquery = "https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" + simbolo
				+ "&outputsize=" + outputsize + "&apikey=" + av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
		...
		for (int i = 0; i < candlesjsontimestamparray.length(); i++) {
			...
			mccandle candleatual = new mccandle(timestampcandle, opencandle, highcandle, closecandle, lowcandle,
					volumecandle);
			listacandlesretornar.add(candleatual);
		}
	...
	public java.util.List<mccandle> av_receberstockcandlesweekly(String simbolo) {
		...
		String urlquery = "https://www.alphavantage.co/query?function=TIME_SERIES_WEEKLY&symbol=" + simbolo + "&apikey="
				+ av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
	...
	public java.util.List<mccandle> av_receberstockcandlesmonthly(String simbolo) {
		...
		String urlquery = "https://www.alphavantage.co/query?function=TIME_SERIES_MONTHLY&symbol=" + simbolo
				+ "&apikey=" + av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
	...
	public java.util.List<mccandle> av_receberforexcandlesintraday(String fromsimbolo, String tosimbolo,
			String intervalo, String outputsize) {
		/*
		 * { "Meta Data": { "1. Information": "FX Intraday (5min) Time Series",
		 * "2. From Symbol": "USD", "3. To Symbol": "BRL", "4. Last Refreshed":
		 * "2019-02-12 17:55:00", "5. Interval": "5min", "6. Output Size": "Compact",
		 * "7. Time Zone": "UTC" }, "Time Series FX (5min)": { "2019-02-12 17:55:00": {
		 * "1. open": "3.7119", "2. high": "3.7128", "3. low": "3.7114", "4. close":
		 * "3.7124" }, "2019-02-12 17:50:00": { "1. open": "3.7154", "2. high":
		 * "3.7161", "3. low": "3.7116", "4. close": "3.7125" },
		 */

		String urlquery = "https://www.alphavantage.co/query?function=FX_INTRADAY&from_symbol=" + fromsimbolo
				+ "&to_symbol=" + tosimbolo + "&interval=" + intervalo + "&outputsize=" + outputsize + "&apikey="
				+ av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
	...
	public java.util.List<mccandle> av_receberforexcandlesdaily(String fromsimbolo, String tosimbolo,
			String outputsize) {
		String urlquery = "https://www.alphavantage.co/query?function=FX_DAILY&from_symbol=" + fromsimbolo
				+ "&to_symbol=" + tosimbolo + "&outputsize=" + outputsize + "&apikey=" + av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
	...
	public java.util.List<mccandle> av_receberforexcandlesweekly(String fromsimbolo, String tosimbolo) {
		String urlquery = "https://www.alphavantage.co/query?function=FX_WEEKLY&from_symbol=" + fromsimbolo
				+ "&to_symbol=" + tosimbolo + "&apikey=" + av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
	...
	public java.util.List<mccandle> av_receberforexcandlesmonthly(String fromsimbolo, String tosimbolo) {
		String urlquery = "https://www.alphavantage.co/query?function=FX_MONTHLY&from_symbol=" + fromsimbolo
				+ "&to_symbol=" + tosimbolo + "&apikey=" + av_chave;
		// mierclasses.mcfuncoeshelper.setarclipboard(urlquery);
		// mierclasses.mcfuncoeshelper.mostrarmensagem(urlquery);
		String jsonconteudo = mwcomms.receberconteudopagina(urlquery);
	...
	public java.util.List<String> crycom_procurarsimbolo(String parametrobusca) {
		/*
		 * { "Response": "Success", "Message": "Coin list succesfully returned!",
		 * "Data": { "42": { "Id": "4321", "Url": "/coins/42/overview", "ImageUrl":
		 * "/media/12318415/42.png", "Name": "42", "Symbol": "42", "CoinName":
		 * "42 Coin", "FullName": "42 Coin (42)", "Algorithm": "Scrypt", "ProofType":
		 * "PoW/PoS", "FullyPremined": "0", "TotalCoinSupply": "42", "BuiltOn": "N/A",
		 * "SmartContractAddress": "N/A", "PreMinedValue": "N/A", "TotalCoinsFreeFloat":
		 * "N/A", "SortOrder": "34", "Sponsored": false, "IsTrading": true,
		 * "TotalCoinsMined": 41.99995653, "BlockNumber": 0, "NetHashesPerSecond": 0,
		 * "BlockReward": 0, "BlockTime": 0 }, "ETH": { "Id": "7605", "Url":
		 * "/coins/eth/overview", "ImageUrl": "/media/20646/eth_logo.png", "Name":
		 * "ETH", "Symbol": "ETH", "CoinName": "Ethereum", "FullName": "Ethereum (ETH)",
		 * "Algorithm": "Ethash", "ProofType": "PoW", "FullyPremined": "0",
		 * "TotalCoinSupply": "0", "BuiltOn": "N/A", "SmartContractAddress": "N/A",
		 * "PreMinedValue": "N/A", "TotalCoinsFreeFloat": "N/A", "SortOrder": "2",
		 * "Sponsored": false, "IsTrading": true, "TotalCoinsMined": 104716377.6866,
		 * "BlockNumber": 7174858, "NetHashesPerSecond": 140129872071527, "BlockReward":
		 * 3, "BlockTime": 15 } }, "BaseImageUrl": "https://www.cryptocompare.com",
		 * "BaseLinkUrl": "https://www.cryptocompare.com", "RateLimit": {},
		 * "HasWarning": false, "Type": 100 }
		 */
	...
	public java.util.List<mccandle> crycom_recebercryptocandles(String simbolo, String limite, String periodo) {
		...
		if (periodo.equals("Daily"))
			caminhourl = "https://min-api.cryptocompare.com/data/histoday?fsym=" + simbolo.toUpperCase()
					+ "&tsym=USD&limit=" + limite + "&api_key=" + crycom_chave;
		else if (periodo.equals("Hourly"))
			caminhourl = "https://min-api.cryptocompare.com/data/histohour?fsym=" + simbolo.toUpperCase()
					+ "&tsym=USD&limit=" + limite + "&api_key=" + crycom_chave;
		else if (periodo.equals("Minute"))
			caminhourl = "https://min-api.cryptocompare.com/data/histominute?fsym=" + simbolo.toUpperCase()
					+ "&tsym=USD&limit=" + limite + "&api_key=" + crycom_chave;
		...
		// mierclasses.mcfuncoeshelper.mostrarmensagem(jsonconteudo);
		communication.mcfuncoeshelper.setarclipboard(jsonconteudo);
		JSONObject obj = new JSONObject(jsonconteudo);
		...
		for (int i = 0; i < datacandles.length(); i++) {
			...
			String timestampcandle = (timestampdate.getYear() + 1900) + "-" + (timestampdate.getMonth() + 1) + "-"
					+ (timestampdate.getDate()) + "-" + (timestampdate.getHours()) + "-" + (timestampdate.getMinutes())
					+ "-" + (timestampdate.getSeconds());

			communication.mccandle candleadd = new communication.mccandle(timestampcandle, cjson_open, cjson_high,
					cjson_close, cjson_low, cjson_volume);
	...
	public java.util.List<String> iex_procurarsimbolo(String parametrobusca) {
		// funcao para retornar uma lista de simbolos encontrados para o parametro de
		// busca em questao

		/*
		 * 
		 * [ { "symbol": "A", "name": "Agilent Technologies Inc.", "date": "2019-01-04",
		 * "isEnabled": true, "type": "cs", "iexId": "2" }, { "symbol": "AA", "name":
		 * "Alcoa Corporation", "date": "2019-01-04", "isEnabled": true, "type": "cs",
		 * "iexId": "12042" }, { "symbol": "AAAU", "name": "Perth Mint Physical Gold",
		 * "date": "2019-01-04", "isEnabled": true, "type": "N/A", "iexId": "14924" }, {
		 * "symbol": "AABA", "name": "Altaba Inc.", "date": "2019-01-04", "isEnabled":
		 * true, "type": "cs", "iexId": "7653" }, { "symbol": "AAC", "name":
		 * "AAC Holdings Inc.", "date": "2019-01-04", "isEnabled": true, "type": "cs",
		 * "iexId": "9169" }, { "symbol": "AADR", "name":
		 * "AdvisorShares Dorsey Wright ADR", "date": "2019-01-04", "isEnabled": true,
		 * "type": "et", "iexId": "5" }, { "symbol": "AAL", "name":
		 * "American Airlines Group Inc.", "date": "2019-01-04", "isEnabled": true,
		 * "type": "cs", "iexId": "8148" }, { "symbol": "AAMC", "name":
		 * "Altisource Asset Management Corp Com", "date": "2019-01-04", "isEnabled":
		 * true, "type": "cs", "iexId": "7760" }, { "symbol": "AAME", "name":
		 * "Atlantic American Corporation", "date": "2019-01-04", "isEnabled": true,
		 * "type": "cs", "iexId": "7" }, { "symbol": "AAN", "name": "Aaron's Inc.",
		 * "date": "2019-01-04", "isEnabled": true, "type": "cs", "iexId": "8" }, {
		 * "symbol": "AAOI", "name": "Applied Optoelectronics Inc.", "date":
		 * "2019-01-04", "isEnabled": true, "type": "cs", "iexId": "7790" }, { "symbol":
		 * "AAON", "name": "AAON Inc.", "date": "2019-01-04", "isEnabled": true, "type":
		 * "cs", "iexId": "9" }, { "symbol": "AAP", "name":
		 * "Advance Auto Parts Inc W/I", "date": "2019-01-04", "isEnabled": true,
		 * "type": "cs", "iexId": "10" }, { "symbol": "AAPL", "name": "Apple Inc.",
		 * "date": "2019-01-04", "isEnabled": true, "type": "cs", "iexId": "11" }, {
		 * "symbol": "AAT", "name": "American Assets Trust Inc.", "date": "2019-01-04",
		 * "isEnabled": true, "type": "cs", "iexId": "12" }, { "symbol": "AAU", "name":
		 * "Almaden Minerals Ltd.", "date": "2019-01-04", "isEnabled": true, "type":
		 * "cs", "iexId": "13" } ]
		 * 
		 */
		...
		for (int i = 0; i < simbolosjson.length(); i++) {
			...
			// String s_isenabled =
			// String.valueOf(candleobjectatual.getBoolean("isEnabled"));
	...
	public java.util.List<mccandle> iex_receberstockcandleswithoutminutes(String simbolo, String periodo) {
		/*
		 * [ { "date": "2014-01-06", "open": 70.4653, "high": 71.6912, "low": 69.9605,
		 * "close": 71.3149, "volume": 103359151, "unadjustedVolume": 14765593,
		 * "change": 0.386777, "changePercent": 0.545, "vwap": 70.9303, "label":
		 * "Jan 6, 14", "changeOverTime": 0 }, { "date": "2014-01-07", "open": 71.366,
		 * "high": 71.5811, "low": 70.5276, "close": 70.8046, "volume": 79432766,
		 * "unadjustedVolume": 11347538, "change": -0.510346, "changePercent": -0.716,
		 * "vwap": 71.7785, "label": "Jan 7, 14", "changeOverTime":
		 * -0.007155587401791223 }, { "date": "2014-01-08", "open": 70.6436, "high":
		 * 71.5286, "low": 70.6279, "close": 71.2533, "volume": 64686685,
		 * "unadjustedVolume": 9240955, "change": 0.448723, "changePercent": 0.634,
		 * "vwap": 71.2531, "label": "Jan 8, 14", "changeOverTime":
		 * -0.000863774610915791 }, { "date": "2014-01-09", "open": 71.6912, "high":
		 * 71.6991, "low": 70.19, "close": 70.3432, "volume": 69905199,
		 * "unadjustedVolume": 9986457, "change": -0.910039, "changePercent": -1.277,
		 * "vwap": 70.8314, "label": "Jan 9, 14", "changeOverTime":
		 * -0.013625483594592414 } ]
		 * 
		 * 
		 */

		String jsonconteudo = mwcomms
				.receberconteudopagina("https://api.iextrading.com/1.0/stock/" + simbolo + "/chart/" + periodo);
		...
		for (int i = 0; i < candlesjson.length(); i++) {
			...
			String timestampcandle = cjson_date.split("-")[0] + "-" + cjson_date.split("-")[1] + "-"
					+ cjson_date.split("-")[2] + "-" + "00" + "-" + "00" + "-" + "00";
	...
	public java.util.List<mccandle> iex_receberstockcandleswithminutes(String simbolo, String periodo) {
		/*
		 * [ { "date": "20190104", "minute": "09:30", "label": "09:30 AM", "high":
		 * 144.54, "low": 144.07, "average": 144.356, "volume": 9329, "notional":
		 * 1346694.07, "numberOfTrades": 95, "marketHigh": 144.58, "marketLow": 144.06,
		 * "marketAverage": 144.451, "marketVolume": 2029196, "marketNotional":
		 * 293119046.0123, "marketNumberOfTrades": 3799, "open": 144.42, "close":
		 * 144.48, "marketOpen": 144.58, "marketClose": 144.51, "changeOverTime": 0,
		 * "marketChangeOverTime": 0 }, { "date": "20190104", "minute": "09:31",
		 * "label": "09:31 AM", "high": 144.49, "low": 143.82, "average": 144.162,
		 * "volume": 14569, "notional": 2100302.925, "numberOfTrades": 79, "marketHigh":
		 * 144.54, "marketLow": 143.6, "marketAverage": 144.035, "marketVolume": 472509,
		 * "marketNotional": 68058023.4464, "marketNumberOfTrades": 3741, "open":
		 * 144.49, "close": 143.91, "marketOpen": 144.509, "marketClose": 143.95,
		 * "changeOverTime": -0.0013438998032640722, "marketChangeOverTime":
		 * -0.0028798692982395196 } ]
		 * 
		 */
		...
		for (int i = 0; i < candlesjson.length(); i++) {
			...
			String anotimestamp = communication.mcfuncoeshelper.retornarSubstringIndices(cjson_date, 0, 3);
			// mierclasses.mcfuncoeshelper.mostrarmensagem(anotimestamp);
			String mestimestamp = communication.mcfuncoeshelper.retornarSubstringIndices(cjson_date, 4, 5);
			// mierclasses.mcfuncoeshelper.mostrarmensagem(mestimestamp);
			String diatimestamp = communication.mcfuncoeshelper.retornarSubstringIndices(cjson_date, 6, 7);
			...
			String timestampcandle = anotimestamp + "-" + mestimestamp + "-" + diatimestamp + "-" + horatimestamp + "-"
					+ minutotimestamp + "-" + "00";
	...
	// the values returned by these functions are not real and used for offline
	// trading and testing only
	public java.util.List<mccandle> offline_receberstockcandlessample() throws IOException {
		File sampleJson = new File(getClass().getClassLoader().getResource("samples/apple5y.json").getFile());
		String data = FileUtils.readFileToString(sampleJson, "UTF-8");

		JSONArray candlesjson = new JSONArray(data);
		java.util.List<mccandle> listacandlesretornar = new java.util.ArrayList<>();
		for (int i = 0; i < candlesjson.length(); i++) {
			JSONObject candleobjectatual = candlesjson.getJSONObject(i);
			String cjson_date = candleobjectatual.getString("date");
	...
	public java.util.List<Double> offline_simularbidaskcandles(java.util.List<communication.mccandle> candlesatual) {
		// funcao para estimar um valor de ask e bid para o simbolo atual, para testar o
		// trader bot
		// mierclasses.mcquote quote = receberquote(simbolo);
		// double ultimoclose = quote.closed;
		// double rangebidaskestimado = 0.0025;
		java.util.List<communication.mccandle> candlessample = candlesatual;
		communication.mccandle ultimacandlesample = candlessample.get(candlessample.size() - 1);
	...
	public java.util.List<mccandle> math_retirarzeroscandles(java.util.List<mccandle> candles) {
		...
		// funcao para retirar candles que possuem valor 0, seja em open, high, low ou
		// close
